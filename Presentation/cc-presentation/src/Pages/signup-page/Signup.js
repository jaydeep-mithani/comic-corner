...
const Signup = () => {
	...
	const [errorMessage, setErrorMessage] = useState("One or more fieds are either empty or invalid. Make sure the below list is checked!");
	...
	const handleFormSubmit = async (e) => {
		...
		if (
			...
			setErrorMessage("One or more fieds are either empty or invalid. Make sure the below list is checked!");
			setErrorMSG(true);
		} else if (
			users
			...
			setErrorMessage('Email "' + email.current.value.toLowerCase() + '" is already in use! Try using another one.');
			setErrorMSG(true);
		} else if (users.map((user) => user.username).includes(username.current.value.toLowerCase())) {
			setErrorMessage(
				'A hero with name "' + username.current.value.toLowerCase() + '" already exist. Try to create your own identity! ;-)'
			);
			setErrorMSG(true);
		} else {
			...
			try {
				const response = await axios.post("http://localhost:1310/user/add", formData);
				...
				console.error(error);
				setErrorMessage("We're sincerely sorry. It seems something went wrong on our end. Please try again later.");
				setErrorMSG(true);
			}
		}
	};
	...
	const handleEmailValidation = (e) => {
		...
		if (e.target.value === "" || regex.test(e.target.value)) setEmailValid(true);
			...
			<div className="container-fluid">
				...
				<div className="container">
					<div className="row g-0">
						<Form className="col-12 col-xxl-6" id="signup-form" method="multipart/form-data" noValidate>
						...
						<div className="col-11 col-xxl-5 mx-auto m-xxl-5 container-fluid rounded rounded-4 shadow bg-dark">
							<div className="col-12 text-start fs-3 text-muted ps-4 pt-3">First Name:</div>
							<ValidSummary isValid={fNameValid}>First name shan't be empty!</ValidSummary>
							<div className="col-12 text-start fs-3 text-muted ps-4 pt-3">Last Name:</div>
							<ValidSummary isValid={lNameValid}>Last name shan't be empty!</ValidSummary>
							...
							<div className="col-12 text-start fs-3 text-muted ps-4 pt-3">confirm password:</div>
							<ValidSummary isValid={pwConf}>Confirm secret code shall be same as the secret code!</ValidSummary>
						</div>
						<span className="mb-5" />
						<ButtonLit
							type="submit"
